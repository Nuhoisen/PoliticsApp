...
ENABLE_CAND_ANOMOLY_DEBUG = False
...
for key,value in name_2_abbrev.items():
    ...
    try:
        ...
        for candidate in cand_list:
            try:
                if candidate["officeName"] == "State Assembly" or candidate["officeName"] == "State House":
        ...
        sorted_sql_cand_list = sorted(sql_cand_list, key = lambda i: i["District"])

        
        # print("Number of SQL Representatives: %d, Number of VS Representatives: %d " % ( len(sorted_sql_cand_list) , len(sorted_vs_cand_list)) )
        ...
        for i in range(0, len(sorted_sql_cand_list)):
            ...
            vs_match_cand = search_for_matching_vs_cand( sql_cand, sorted_vs_cand_list)
            ...
            if vs_match_cand:
                ...
                # If not, check if their first name is the issue
                    ...
                    if iterative_levenshtein(sql_cand_name, vs_cand_name) < 5:
                        ...
                        if ENABLE_CANDIDATE_DEBUG and ENABLE_CAND_ANOMOLY_DEBUG:
                            print( "ANOMOLY FOUND: \n\tVotesmart Cand: %s, \tSQL Cand: %s, \n\tState: %s, \n\tPosition: %s, \n\tDistrict: %s , \n\tVoteSmartID: %s" %  (vs_cand_name, sql_cand_name, key, vs_match_cand["officeName"], vs_match_cand["officeDistrictName"], vs_match_cand["candidateId"] ) )
