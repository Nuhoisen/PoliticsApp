...
def search_for_matching_vs_cand( sql_cand, sorted_vs_cand_list):
    ...
    for j in range( 0 , len( sorted_vs_cand_list ) ):
        ...
        if int( vs_cand["officeDistrictName"] ) ==  int( sql_cand["District"] ) :
...
for key,value in name_2_abbrev.items():
    ...
    try:
        ...
        for candidate in cand_list:
            try:
                ...
                if candidate["officeName"] == "State House" :
                    ...
                    # Specially Tailored to Washington
                    # First check district 1, then check district 2
                    if "A" in candidate["officeDistrictName"] or "B" in candidate["officeDistrictName"] or "C" in candidate["officeDistrictName"]:
                            
                        # Just take the number from the snippet
                        candidate["officeDistrictName"]  = candidate["officeDistrictName"][: -1]
        ...
        sorted_vs_cand_list = refined_vs_cand_list #sorted(refined_cand_list, key = lambda i: int(i["officeDistrictName"]))
        ...
        for row in sql_results:
            try:
                ...
                if "A" in cand_district_num or "B" in cand_district_num or "C" in cand_district_num:
                    # Just take the number from the snippet
                    cand_district_num  = cand_district_num[: -1]
        ...
        sorted_sql_cand_list = sql_cand_list #sorted(sql_cand_list, key = lambda i: i["District"])
