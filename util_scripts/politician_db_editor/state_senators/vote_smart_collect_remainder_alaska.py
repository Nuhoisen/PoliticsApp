...
COMMIT_EXACT_MATCHES = True
COMMIT_CLOSE_MATCHES = True


# Print a message for every entry checked
DEBUG_SQL_UPDATE_ENTRIES = True
...
def search_for_matching_vs_cand( sql_cand, sorted_vs_cand_list):
    ...
    for j in range( 0 , len( sorted_vs_cand_list ) ):
        ...
        if vs_cand["officeDistrictName"]  ==   sql_cand["District"]  :
           vs_cand_list.append( vs_cand )
    # No match
    return vs_cand_list
...
for key,value in name_2_abbrev.items():
    ...
    try:
        ...
        ####################################################
        ####################################################
        # Extract just the senators
        for candidate in cand_list:
            try:
                if candidate["officeName"] == "State Senate":
        ...
        sorted_vs_cand_list = refined_cand_list
        #sorted(refined_cand_list, key = lambda i: int(i["officeDistrictName"]))
        ...
        for row in sql_results:
            try:
                # Get just the district letter
                cand_district_letter   = row.District.split()[-1]
                cand_name           = row.Name
                
                temp["Id"]          = row.Id
                temp["LastName"]    = cand_name.split()[-1]
                temp["Name"]        = cand_name
                temp["District"]    = cand_district_letter
        ...
        # List cannot be sorted numerically
        sorted_sql_cand_list = sql_cand_list
        # sorted(sql_cand_list, key = lambda i: i["District"])
        ...
        for i in range(0, len(sorted_sql_cand_list)):
            ...
            possible_vs_cands = search_for_matching_vs_cand( sql_cand, sorted_vs_cand_list)
            
            # Debugging message
            if not possible_vs_cands:
                print("NO MATCHING DISTRICT FOUND FOR: %s , State: %s , District: %s" % ( sql_cand["Name"], key, sql_cand["District"] ) )


            for vs_match_cand in possible_vs_cands:
