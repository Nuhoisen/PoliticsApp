...
def run():    
    passed = 0
    failed = 0
    for i in range(20000, 25000):    #RUN FROM HERE
        try:
            billtrack = url+str(i)
            ...
            if("yes" in in_office.lower()):
                ...
                role = get_role(soup).lstrip()
                role = role.rstrip()
                state = get_state(soup).lstrip()            
                state = state.rstrip()
                district = get_district(soup).lstrip()
                district = district.rstrip()
                dir_path = ".\\profile_images\\" + state+"\\"+role+"\\"+district
                dir_path = dir_path.replace(' ', '_').lower()
                # print(dir_path)
                try:
                    os.makedirs(dir_path)
                    # dir_path = ".\\profile_images\\"+ state
                ...
                urllib.request.urlretrieve("https://www.billtrack50.com/Handlers/LegislatorImageHandler.ashx?id="+str(i), dir_path+"\\"+name+".png")
                # print(role)
                # print(state)
                # print(district)
...
# urllib.urlretrieve("https://www.billtrack50.com/Handlers/LegislatorImageHandler.ashx?id=1", "test.png")