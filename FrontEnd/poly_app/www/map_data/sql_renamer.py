...
#Kansas US Senate        
    ...
    def rename_assembly_districts(self, state):
        cmd = "SELECT District FROM [PoliticianInfo].[dbo].[PoliticianTable] WHERE State='"+ state + "' AND District LIKE'"+state+" Assembly District 0%'"
        print(cmd)
        cursor = self.c_cnxn.cursor()
        response = cursor.execute(cmd).fetchall()
        ...
        for line in response:
            ...
            new_district = state +" Assembly District " + number
            # print(old_district)
            # print(new_district)
